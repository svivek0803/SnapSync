//
//  MetalVideoWritter.swift
//  SnapSync
//
//  Created by Vivek Singh on 12/06/22.
//

import Foundation
import AVFoundation

public typealias MetalVideoWriterStart = (CMTime) -> Void

public typealias MetalVideoWriterProgress = (MetalVideoWriterProgressType) -> Void

public enum MetalVideoWriterProgressType {
    case video(CMTime, Bool)
    case audio(CMTime, Bool)
}

/// Video writer writing video file
public class MetalVideoWriter {
    /// URL of video file
    public var url: URL
    /// Video frame size
    public let frameSize: MetalIntSize
    /// Video file type
    ...
    private var progress: MetalVideoWriterProgress?
    
    private var startHandler: MetalVideoWriterStart?
    ...
    public init(
        ...
        let library = try! MetalDevice.sharedDevice.makeDefaultLibrary(bundle: Bundle(for: MetalVideoWriter.self))
        ...
        outputTexture = MetalDevice.sharedDevice.makeTexture(descriptor: descriptor)
    ...
    public func start(startHandler: MetalVideoWriterStart? = nil, progress: MetalVideoWriterProgress? = nil) {
        lock.wait()
        defer { lock.signal() }
...
extension MetalVideoWriter: MetalImageConsumer {
    ...
    public func newTextureAvailable(_ texture: MetalTexture, from source: MetalImageSource) {
        ...
        guard let commandBuffer = MetalDevice.sharedCommandQueue.makeCommandBuffer(),
...
extension MetalVideoWriter: MetalAudioConsumer {
