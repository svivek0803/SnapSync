...
public class MetalView: MTKView {
    ...
    private lazy var renderPipeline: MTLRenderPipelineState = {
        let library = try! device!.makeDefaultLibrary(bundle: Bundle(for: MetalView.self))
    ...
    public override init(frame frameRect: CGRect, device: MTLDevice?) {
        ...
        super.init(frame: frameRect, device: device ?? MetalDevice.sharedDevice)
        super.isPaused = true
    }
    ...
    required init(coder: NSCoder) {
        ...
        super.device = MetalDevice.sharedDevice
        super.isPaused = true
    ...
    public override func draw(_ rect: CGRect) {
        guard let texture = self.texture,
            ...
            let commandBuffer = MetalDevice.sharedCommandQueue.makeCommandBuffer() else { return }
...
extension MetalView: MetalImageConsumer {
